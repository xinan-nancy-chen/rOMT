function vtkwrite_spdlines( filename,dataType,varargin )
...
switch upper(dataType)        
    ...
    case 'POLYDATA'
        ...
        fprintf(fid, 'ASCII\n');
        if numel(varargin)<3, error('Not enough input arguments'); end
        fprintf(fid, 'DATASET POLYDATA\n');
        
        countpts = zeros(1,size(varargin{2},2));
        x = []; y = x; z = x;
        for plN = 1:size(varargin{2},2)
            xtmp = varargin{2}{plN}(:,1);
            ytmp = varargin{2}{plN}(:,2);
        ...
        switch upper(varargin{1})
            case 'LINES'
                ...
                rr = [1,countsum+1];
                for plN = 1:size(varargin{2},2)
                    ptsv(rr(plN):rr(plN)+countpts(plN)-1) = varargin{3}{plN};%linspace(-1,1,countpts(plN));
                end
